...
def predict(X, w, y=None, normazlized = None):
    ...
    # y_hat =  (y_hat - np.mean(y_hat)) / np.std(y_hat) # predicted ( normalized)
    
    if normazlized== False:
        #y_hat = (y_hat * np.std(y_hat))+np.mean(y_hat)
        #y = (y * np.std(y))+np.mean(y)
        y = y* std_y + mean_y
...
# print("Last training loss: ", losses_train[-1])
